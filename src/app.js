...
function draw(){ctx.clearRect(0,0,canvas.width,canvas.height);ctx.fillStyle='#f8fafc';ctx.fillRect(0,0,canvas.width,canvas.height);for(const z of venue.zones){const[x,y,w,h]=rect(z);ctx.fillStyle=z.type==='blocked'?'#d9dde3':z.type==='destination'?'#cdeccf':z.type==='spawn'?'#cfe3ff':'#e4f3dc';ctx.fillRect(x,y,w,h);ctx.strokeStyle='#7b8491';ctx.strokeRect(x,y,w,h);ctx.fillStyle='#303640';ctx.font='12px system-ui';ctx.textAlign='center';ctx.fillText(z.label,x+w/2,y+h/2+4)}drawRoute();for(const a of agents){if(a.done)continue;const[x,y]=point(a.x,a.y);ctx.beginPath();ctx.arc(x,y,3.2,0,Math.PI*2);ctx.fillStyle='#1976d2';ctx.fill()}}
